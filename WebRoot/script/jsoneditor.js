...
Node.prototype.showContextMenu = function (anchor, onClose) {
    ...
    if (this.parent && this.parent._hasChilds()) {
		...
		items.push({
            'text': 'CtripModel',
            'title': 'Insert CtripModel into this field',
            'className': 'insert',
            'click': function () {
            	//alert("asdfasdfs");
               // node._onDuplicate();
            	//openSubForm();
            	mini.open({
                    url: "newsub/modelForm.jsp",
                    title: "选择Model", width: 700, height: 400,
                    onload: function () {
                        //var iframe = this.getIFrameEl();
                        //var data = { action: "edit", id: 74};
                        //iframe.contentWindow.SetData(data);
                    	//var parent = node.parent;
                    	var className ;
                    	if(node.parent.type=='array')
                    	{
                    		className = node.parent.getField();
                    	}
                    	else
                    	{
                    		className = node.getField();
                    	}
                    	var iframe = this.getIFrameEl();
                    	iframe.contentWindow.SetData(className);
                    },
                    ondestroy: function (action) {
                    	//   var newNode = node.clone();
                    	//   var obj = {"categoryId":2,"categoryName":"海外","cityList":[{"cityId":1,"cityName":"北京","cityNameEn":"Beijing","hotelCount":20,"hotelCountText":"20家酒店","realServiceCode":"17000701"},{"cityId":2,"cityName":"上海","cityNameEn":"Shanghai","hotelCount":20,"hotelCountText":"20家酒店","realServiceCode":"17000701"},{"cityId":3,"cityName":"重庆","cityNameEn":"Chongqing","hotelCount":30,"hotelCountText":"20家酒店","realServiceCode":"17000701"}],"realServiceCode":"17000701"};
                    	//   newNode.setValue(obj ,'object')
                    	//   var clone =	node.parent.insertAfter(newNode,node);
